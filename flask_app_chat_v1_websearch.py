    ...
    try:
        ...
        try:
            ...
            if not all([lef_min, lef_mle, lef_max, lm_min, lm_mle, lm_max]):
                ...
                return render_template('error.html', 
                    error=f"Missing required fields: {', '.join(missing)}. Please complete all estimate fields."), 400
            ...
            return render_template('error.html', 
                error="Invalid number format. Please enter valid numbers."), 400
        ...
        if not (0 <= original_inputs['lef_min'] <= original_inputs['lef_mle'] <= original_inputs['lef_max']):
            logger.error(f"[{VERSION}] Invalid LEF range")
            return render_template('error.html', 
                error=f"Invalid frequency estimates: min ≤ most likely ≤ max not satisfied"), 400
        
        if not (0 <= original_inputs['lm_min'] <= original_inputs['lm_mle'] <= original_inputs['lm_max']):
            logger.error(f"[{VERSION}] Invalid LM range")
            return render_template('error.html', 
                error=f"Invalid magnitude estimates: min ≤ most likely ≤ max not satisfied"), 400
        
        logger.info(f"[{VERSION}] Running Monte Carlo simulation")
        ...
        if missing_keys:
            logger.error(f"[{VERSION}] Simulation returned invalid results")
            return render_template('error.html',
                error=f"Simulation error: Invalid results format"), 500
        
        logger.info(f"[{VERSION}] Simulation complete: Mean=${results['mean']:,.0f}")
        ...
        return render_template('error.html', error=f"Error during analysis: {str(e)}"), 500
...
def recalculate():
    ...
    try:
        ...
        logger.info(f"[{VERSION}] Recalculating with reductions")
        new_results = run_monte_carlo(**adjusted_inputs, n_simulations=n_simulations)
        
        return jsonify({'status': 'success', 'results': new_results})
        
    except Exception as e:
        logger.error(f"[{VERSION}] Recalculation error: {e}")
        return jsonify({'error': 'Recalculation failed', 'details': str(e)}), 500
